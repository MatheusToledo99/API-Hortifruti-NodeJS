...
export default class EnderecosController {
  public async store({ request, response, auth }: HttpContextContract) {
    ...
    if (!cliente) {
      return response.badRequest({
        message: "Somente usuários clientes podem cadastrar endereço.",
      });
    }
  ...
  public async update({
    ...
    if (userAuth.id !== cliente.userId) {
      return response.unauthorized({
        message: "Você não está autorizado a alterar este registro.",
      });
    }
  ...
  public async destroy({ params, response, auth }: HttpContextContract) {
    ...
    try {
      ...
      const cliente = await Cliente.findByOrFail(
        "id",
        enderecoDelete.clienteId
      );

      if (userAuth.id !== cliente.userId) {
        return response.unauthorized({
          message: "Você não está autorizado a excluir este registro.",
        });
      }
